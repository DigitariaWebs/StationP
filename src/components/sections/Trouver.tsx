...
export default function AboutPage() {
  return (
    <section
      id="trouveruneborne"
      className="bg-[color:var(--color-background)] text-[color:var(--color-foreground)] relative"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 sm:py-14 w-full">
        <section className="mb-8">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-black tracking-tighter leading-[0.95]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-4">
          <div className="md:col-span-5 bg-[color:var(--color-surface-muted)] border border-[color:var(--color-border)] rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8">
            <h3 className="text-2xl sm:text-3xl font-black mb-4">Constat</h3>
            <ul className="space-y-3 text-base sm:text-lg text-[color:var(--color-muted)] font-medium">
          ...
          <div className="md:col-span-7 bg-[color:var(--color-accent)] rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 text-[color:var(--color-accent-contrast)] flex flex-col lg:flex-row gap-6 overflow-hidden relative">
            <div className="flex-1">
              <h3 className="text-3xl sm:text-4xl font-black mb-4 flex items-center gap-3">
                <Zap size={32} fill="currentColor" /> Solution
              </h3>

              <p className="text-base sm:text-lg font-bold mb-4">
                Une communauté locale qui permet de :
              </p>
              <ul className="space-y-3 text-base sm:text-lg font-bold">
                <li className="flex items-start gap-3"><Home size={20} className="mt-0.5" /> partager sa borne de recharge</li>
                <li className="flex items-start gap-3"><Users size={20} className="mt-0.5" /> recharger près de chez soi</li>
                <li className="flex items-start gap-3"><Zap size={20} className="mt-0.5" /> recharger à moindre coût</li>
              </ul>
            </div>
        ...
        <section className="bg-[color:var(--color-surface-strong)]/40 backdrop-blur-md border border-[color:var(--color-border)] rounded-[1.5rem] sm:rounded-[2rem] p-3 sm:p-5">
          <h3 className="font-black text-sm sm:text-lg uppercase tracking-tight mb-2">Comment ça marche</h3>
          ...
          <a
            href="#howitworks"
            className="group mt-6 w-full sm:w-auto inline-flex items-center justify-center gap-3 bg-[color:var(--color-foreground)] text-[color:var(--color-background)] px-6 py-3 rounded-full font-black text-base sm:text-lg uppercase tracking-widest hover:scale-105 transition-all"
          >
...
function Step({ icon, title, desc }: { icon: string; title: string; desc: string }) {
  return (
    <div className="bg-[color:var(--color-surface-muted)] border border-[color:var(--color-border)] p-3 rounded-[1.25rem] flex items-center gap-2">
      ...
      <div>
        <h4 className="font-black text-xs sm:text-sm text-[color:var(--color-foreground)] uppercase leading-none mb-1">{title}</h4>
        <p className="text-xs text-[color:var(--color-muted)] font-medium leading-tight">{desc}</p>
      </div>
    </div>
  );
}
